...
import { Target, Sparkles, Send, ArrowLeft, Flame } from 'lucide-react';
import { useStore } from '../store/useStore';
import toast from 'react-hot-toast';

export default function LeadCapture() {
  const { eventId } = useParams();
  const { events, addLead } = useStore();
  ...
  const resetForm = () => {
    ...
    setForm({ name: '', email: '', phone: '', company: '', designation: 'Founder / CXO', interest: 'Enterprise CRM / Growth Tech', budget: '> ₹5,00,000', timeline: 'Immediate (0-30 days)', notes: '' });
  };
  ...
  const handleSubmit = async (e) => {
    ...
    setSubmitting(true);
    try {
      const leadRecord = await addLead({
        ...form,
        eventId: event?.id,
        eventTitle: event?.title,
      });
      setGeneratedScore(leadRecord);
      toast.success(`🎯 Lead Captured! AI Score: ${leadRecord.score}/100`);
    } catch (err) {
      toast.error(err.message || 'Failed to capture lead. Please try again.');
    } finally {
      setSubmitting(false);
    }
  };
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                ...
                <div>
                  ...
                  <input value={form.company} onChange={e => setForm({ ...form, company: e.target.value })} placeholder="e.g. XYZ Pvt Ltd"
                    className="w-full border border-slate-200 rounded-xl px-4 py-2.5 text-sm focus:ring-2 focus:ring-blue-600 focus:outline-none" />
                </div>
              </div>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div>
                  ...
                  <input type="email" value={form.email} onChange={e => setForm({ ...form, email: e.target.value })} placeholder="rahul@xyzpvt.com"
                    className="w-full border border-slate-200 rounded-xl px-4 py-2.5 text-sm focus:ring-2 focus:ring-blue-600 focus:outline-none" />
                </div>
                <div>
              ...
              <button type="submit" disabled={submitting}
                className="w-full flex items-center justify-center gap-2 bg-blue-700 hover:bg-blue-800 text-white font-bold py-3 rounded-xl transition shadow-lg shadow-blue-700/20 disabled:opacity-60">
                <Send className="w-4 h-4" /> {submitting ? 'Qualifying Lead...' : 'Qualify Lead & Assign AI Score'}
              </button>
            </form>
          </div>
            ...
            {generatedScore ? (
              <div className="bg-white rounded-3xl border border-emerald-200 p-6 shadow-sm text-center space-y-4">
                ...
                <div className="space-y-2 pt-2">
                  <button onClick={() => toast.success('⚡ Lead assigned to B2B Sales Rep with 24h SLA!')}
